...
위 명령어를 실행하면, 서브모듈이 현재 디렉토리에 포함이 된다. 서브모듈을 추가하면 `.gitmodules` 파일이 생성된다. 이 파일에는 서브모듈에 대한 정보가 포함되어 있다.
...
`Git` 서브모듈은 하나의 프로젝트에서 별도의 `Git` 저장소를 함께 관리할 수 있는 기능이다. 하지만 서브모듈을 사용할 때는 다음과 같은 점을 유의해야 한다.
