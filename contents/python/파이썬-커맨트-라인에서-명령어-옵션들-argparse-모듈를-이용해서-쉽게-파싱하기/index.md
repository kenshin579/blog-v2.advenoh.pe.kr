...
셀이나 리눅스 명령어를 실행할 때 많은 옵션이 존재한다. 아래는 pip 명령어(파이썬 패키지 관리자)의 옵션 목록이다. Flag 형태의 옵션(ex. --no-color)이나 입력값을 받을 수 있는 옵션(ex. --log <path>)도 있다.

![](image_6.png)

이런 옵션을 파이썬에서 구현하려면 어떻게 해야 할까요? 실제 구현한다면, 실행 명령어를 인자로 받아서 parse 하는 과정이 필요한다. 직접 구현하기는 좀 부담스럽죠. 셀이나 여러 언어에서 이런 부분들을 별도의 모듈로 제공한다. 파이썬에서는 커맨드 parsing 라이브러리로 getopt, argparse, docopt가 있다. 이 중에서 파이썬에서 많이 사용되는 **argparse** 를 알아보도록 하자.

# 2. argparse 옵션 알아보기

argparse는 파이썬 표준 라이브러리에 포함되어 있어 별도로 설치할 필요는 없다. 모듈을 사용하기 위해 import로 추가만 해주면 된다.
...
이 외에도 다양한 옵션 설정이 있지만, 자주 사용되는 옵션으로만 정리해보았다.
