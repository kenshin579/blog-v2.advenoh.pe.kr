...
헤로쿠(Heroku)는 [PaaS](https://blogs.msdn.microsoft.com/eva/?p=1383)(Platform as a Service)형태의 클라우드 서비스이다. 헤로쿠는 터미널이나 웹에서 필요한 여러 티어(ex. DB)를 쉽게 생성하고 연동시킬 수 있다. 최초 버전에서는 Ruby 언어만 지원하였지만, 현재는 메이저급 언어 대부분을 지원하고 있다.
...
먼저 [Heroku](http://www.heroku.com/) 사이트에 들어가 계정을 만들어야 한다.

## 2.2 Heroku CLI 설치

터미널에서 Heroku 앱을 관리할 수 있도록 Heroku CLI (Command Line Interface)를 설치한다. 이 포스팅에서는 맥 기반으로 설명되어 있다. 그외에 OS는 [Heroku 설치 가이드](https://devcenter.heroku.com/articles/heroku-cli)를 참조해주세요.
...
Heroku관련 작업을 하려면 먼저 로그인 명령어로 Heroku 계정에 로그인해야 한다.
...
Heroku를 배포할때 git를 사용하기 때문에 git가 있어야 한다. 맥 OS에서는 Xcode가 설치되어 있으면 기본적으로 터미널에서 사용할 수 있다.
...
실제 개발과정을 보면 처음에 프로젝트 생성한 이후 코드 수정, 개발환경에서의 테스트, 서버로 배포하는 이런 과정을 계속 반복하게 된다. Heroku 클라우드에 개발하는 코드 어떻게 쉽게 반영할 수 있는지 알아보죠.

## 3.1 Sample 프로젝트 생성

Heroku에서 기본적으로 제공하는 샘플 프로젝트를 다운로드한다.
...
먼저 create 명령어로 Heroku에 관련 Git 저장소와 빈 앱을 생성한다.
...
```
이제 실제로 Heroku 클라우드에 배포해보죠. Git 명령어로 Heroku 저장소에 푸시하면 배포가 완료된다. 참 쉽죠? ㅋㅋ
...
소스가 올라가면 빌드 과정을 거치고 공개 주소에 접속해 웹 앱에 접속할 수 있다. 빌드 과정이후 Heroku에서 웹 앱을 시작하는 시작점은 Procfile 파일에 정의되어 있다.
...
로컬환경에서 수정이 잘됐는지 확인 이후 이상없으면 코드를 commit한다.
...
Heroku에 배포하고 반영 잘됐는지 브라우저를 오픈해서 확인한다. 변경한 타이틀이 잘 로드가 되네요.
...
명령어외에도 [직접 마켓장소](https://elements.heroku.com/addons)에서 접속해서 add-on을 추가할 수 있다.
...
앱에서 MongoDB로 접속하는 데 필요한 파일을 생성하고 기존 코드를 수정한다. 원하는 document 구조에 맞게 schema 파일을 생성한다.
...
메인 index.js 파일을 수정한다. RESTful API /timers 접속 시 모든 데이터를 조회 가능하도록 구현한다.
...
새로 얻은 MongoDB 주소를 코드에 반영하고 MongoDB에 dummy 데이터를 로드한다.

```bash
$ node data/populate.js
```

로컬 환경에서 이상이 없으면, commit해서 Heroku에 배포한다.
...
* heroku ps - 실행중인 process를 볼수 있다. 현재 남아 있는 dyno 시간도 알 수 있다
