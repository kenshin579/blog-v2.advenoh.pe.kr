...
description: "추가된 LOG를 JUnit에서 확인하는 방법"
date: 2019-11-18
update: 2019-11-18
tags:
  - java
...
Unit Test를 작성할 때 메서드의 결과를 기본적으로 확인하여 로직을 검증한다. void인 메서드인 경우에는 내부 메서드에서 실행하는 메서드의 실행 여부나 메서드로 넘겨진 인자 값을 가지고 확인하기도 한다.

개발 하다 보면 로직에 변화는 없지만, 단순히 메서드에서 로그를 추가하는 경우가 생깁니다. 개인적으로는 추가한 로그를 Unit Test로 꼭 확인해야 하는 건 아니지만, 확인이 가능한지 궁금증이 생겨서 알아보았다.
...
개발하는 프로젝트에서 아래와 같이 예외처리가 안 된 부분에 로그를 추가하여 원하는 정보를 출력하도록 하였다. catch 문구에 추가된 로그를 Unit Test에서 확인해보자.
...
ListAppender 클래스는 발생하는 Log 이벤트를 List에 저장하여 나중에 조회 가능하도록 해주는 클래스이다.
...
someService.requestJobId() 메서드에서 로그를 찍게 되고 로그로 출력한 내용은 listAppender에 저장된다. 저장한 로그의 메시지를 확인하기 위해 listAppender.list에서 List를 가져옵니다.
