...
기업에서도 그렇고 이제 아마존 서비스를 쓰지 않은 곳이 없을 정도로 회사마다 아마존의 서비스를 많이 사용하고 있다. 최근에 이직을 한 곳에서도 S3 (Simple Storage Service) 스토리지 서비스를 이용하고 있어 S3 API를 학습할 겸 해서 정리를 해봤다.
...
- S3 서비스 특징
    ...
    - 다른 아마존 서비스(ex. CloudFront, Glacier)에 쉽게 연동이 가능하다
...
먼저 버킷을 생성해보자. 간단하다. createBucket() 메서드를 사용하면 된다.
...
버킷에서 하나의 파일을 삭제할 때는 deleteObject() 메서드를 사용하고 여러 파일을 한 번에 삭제하려면 deleteObjects() 메서드를 사용한다.
