...
위와 같이 final 키워드를 떠올릴 때면 그냥 상수로만 생각할 때가 종종 있다. final을 클래스, 메서드, 변수에 선언하면 조금씩 할 수 있는 부분들이 제한된다. 너무 당연한 내용이지만, 시간이 지나니까 기억에서 사라져버려서 이번에 다시 한번 상기하기 위해 정리를 해보았다.

자바에서 final 키워드는 여러 컨텍스트에서 단 한 번만 할당될 수 있는 entity를 정의할 때 사용된다. ( [위키피디아](https://en.wikipedia.org/wiki/Final_%28Java%29) )
...
객체 변수에 final로 선언하면 그 변수에 다른 참조 값을 지정할 수 없다. 원시 타입과 동일하게 한번 쓰여진 변수는 재변경 불가한다. **단, 객체 자체가 immutable하다는 의미는 아닙니다**. 객체의 속성은 변경 가능하다.
