...
- type: blank
  ...
  explain: "time.After다. 지정한 시간이 지나면 값을 보내는 channel을 반환하므로, select의 한 case로 두면 다른 case가 그 안에 준비되지 않았을 때 timeout 분기를 타게 된다. 취소 전파까지 필요하면 context.WithTimeout을 쓴다. (3.1절)"
