...
이번 포스팅에서는 Node.js 모듈을 NPM 저장소에 배포하는 방법에 대해서 알아보자. NPM은 Node Package Modules 약자로 Node.js 모듈 관리해주는 패키지 관리자이다.
NPM의 명령어로 쉽게 Node.js 모듈을 설치해서 js 라이브러리를 사용할 수도 있고 또한 개발한 모듈을 다른 이들이 사용할 수 있도록 레지스토리에 배포할 수도 있다.
...
NPM 명령어는 Node.js 설치 시 같이 설치되는데, 노드가 아직 설치되어 있지 않다면 brew로 설치한다.
...
아래는 [app-timer-pomodoro](https://github.com/kenshin579/app-timer-pomodoro) 의 package.json 파일이다. 기본 값으로 설정한 것보다 더 많은 내용이 있지만, 쉽게 이해할 수 있는 수준이다.
...
npm init으로 간단하게 생성한 예제에서는 index.js 소스 파일만 생성해서 ‘hello world’를 출력하자.
...
NPM 리파지토리에 배포하기 위해서는 NPM 사이트에 계정이 있어야 하고 명령어 창에서 사이트로 로그인 이후에 배포할 수 있다.

### 2.4.1 사용자 등록하기

NPMJS 사이트에 계정이 없는 경우에는 [사이트](https://www.npmjs.com/) 에 접속해서 계정을 생성한다.
...
이상 없이 배포되면 NPM 사이트에서도 바로 확인할 수 있다.
...
코드 수정이후에 모듈을 재배포하려면 버전을 업데이트해야 한다. 버저닝은 npm version 명령어를 사용해서 쉽게 업데이트를 하면 된다.
...
버전에서 patch 부분을 업데이트한다.

```bash
$ npm version patch
```

소스 수정하고 테스트하고 버저닝 올리고 배포하는 명령어를 일일이 타이팅하기보다는 배포하는 전체 과정을 package.json에서 스크립트화 하면 보다 전체 재배포 과정을 수월하게 진행할 수 있다. app-timer-pomodoro의 package.json 파일이다.
...
버저닝 이후에는 다시 npm publish로 배포하면 된다.

## 2.6 리파지토리에서 제거하기

이미 publish한 패키지도 unpublish 명령어로 리파지토리에서 제거할 수 있다.
...
.npmignore 파일에 리스트업된 파일이나 폴더는 배포에서 제외된다. .npmignore 파일이 없는 경우에는 .gitignore 파일을 대신 참조하여 배포에서 제외한다.

제외시킬 파일들은 IDE 설정파일, test 관련 파일, log 파일 등이 있다.
