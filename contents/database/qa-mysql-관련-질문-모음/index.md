...
MySQL의 Storage Engine에는 여러 가지가 존재한다. 제일 많이 사용되는 버전은 MyISAM와 InnoDB이다. 테이블 생성시 어떤 엔진을 사용할 지 설정할 수 있다.
...
사용자 정의 변수를 저장할 때 사용한다. 이 경우에는 SELECT로 구한 media_no 값을 mediaNo 변수에 저장한다.
...
MySql 설정에서 general_log을 활성화시켜면 된다.
