...
`QoS`(Quality of Service)는 메시지의 **전달 보장 수준**을 의미한다. 네트워크 상태와 메시지의 중요도에 따라 `QoS`를 고른다. `QoS` 수준이 높을수록 메시지 전달은 더 확실해지지만, 그만큼 네트워크 오버헤드와 지연이 증가한다.

이 장에서는 각 `QoS` 레벨의 동작 원리, `QoS` 선택 기준, 그리고 `QoS` 1에서 발생할 수 있는 중복 메시지 처리 방법을 다룬다.
...
`QoS` 1은 메시지 전달을 보장하지만, `PUBACK`이 유실되면 Publisher가 같은 메시지를 재전송하여 중복이 발생할 수 있다. 이는 `QoS` 1의 설계상 의도된 동작이므로, Subscriber 측에서 중복 처리 작업이 필요하다.
...
중복 메시지를 받아도 결과가 동일하도록 설계하는 것이 **멱등성(Idempotency)**이다. `QoS` 2의 오버헤드 없이도 "정확히 한 번" 처리와 같은 효과를 내므로, 실무에서는 `QoS` 1 + 멱등성 조합을 가장 많이 쓴다.
...
`MQTT`에서 세션(Session)은 `TCP` 연결과는 별개의 개념이다. 세션에는 구독 정보, 전달되지 않은 메시지, `QoS` 흐름 상태 등이 포함된다. 이 장에서는 세션의 생명주기와 Keep Alive 메커니즘, 그리고 Retained Message 활용법을 다룬다.

## 2.1 Session Expiry Interval

세션은 Client와 Broker 간의 **연결 상태 정보**이다. v5에서는 Session Expiry Interval로 연결이 끊어진 후에도 세션을 얼마나 유지할지 제어할 수 있다. 모바일 앱처럼 연결이 자주 끊어지는 환경에서 특히 유용하다.

### 2.1.1 Clean Start vs Session 유지

Clean Start 플래그는 연결 시 이전 세션을 어떻게 처리할지 결정한다.
...
Session이 유지되는 동안 Broker가 메시지를 저장한다. Client가 오프라인이더라도 세션이 살아있으면 `QoS` 1 이상의 메시지는 Broker에 쌓이고, 재연결 시 한꺼번에 전달된다.
...
연결이 살아있는지 확인하는 메커니즘이다. `TCP` 연결은 상대방이 비정상 종료되어도 이를 즉시 감지하지 못하는 경우가 많기 때문에, `MQTT`는 주기적으로 `PINGREQ`/`PINGRESP`를 교환하여 연결 상태를 확인한다.
...
Topic에 **마지막 메시지를 저장**하는 기능이다. Broker가 해당 Topic의 가장 최근 메시지를 보관하고 있다가, 새로운 Subscriber가 구독하면 즉시 전달한다. Subscriber는 Publisher의 다음 발행을 기다리지 않고도 현재 상태를 바로 알 수 있다.
...
많은 `MQTT` 튜토리얼이 연결과 메시지 전송만 다루지만, 실제 프로덕션 코드에서는 재연결 로직이 전체 코드의 상당 부분을 차지한다. 네트워크는 반드시 끊기기 때문이다. 이 장에서는 재연결이 필요한 이유, 재연결 시 발생하는 문제들, 그리고 검증된 재연결 전략을 다룬다.

## 3.1 재연결이 반드시 필요한 이유

### 3.1.1 현실 세계의 네트워크

네트워크 연결은 다양한 이유로 끊어진다. 끊김은 정상적인 운영 환경에서 늘 일어나는 일이므로, 재연결은 예외 처리가 아니라 핵심 기능으로 설계한다.
...
**결론**: 연결 끊김은 일어날지 여부가 아니라 언제 일어나는지의 문제다. 재연결은 반드시 구현해야 한다.
...
sequenceDiagram
    ...
    B->>C: PUBLISH (msg-1)
    B->>C: PUBLISH (msg-2)
    Note over B,C: msg-1은 네트워크에서 유실, msg-2는 정상 수신
    C-->>B: PUBACK (msg-2)
...
**재연결 전략**
- 네트워크 끊김은 언제 일어나는지의 문제이므로 재연결은 필수
- Exponential Backoff + Jitter로 Broker 부하 분산
- 재연결 후 재구독, 중복 체크, 상태 동기화 필수

실무에서는 재연결 로직이 전체 코드의 상당 부분을 차지한다. QoS, Session, 재연결 이 세 가지가 안정적인 `MQTT` 시스템의 토대다.
