...
아래 표는 Wi-Fi 기술이 세대를 거치며 발전해 온 흐름을 정리한 것이다. 속도와 주파수 대역, 핵심 기능의 변화를 보면 각 표준의 특징을 비교하기 좋다.
...
`802.11k`는 모바일 기기가 더 나은 신호의 인접 `AP`를 쉽게 찾도록 돕는 무선 자원 측정(Radio Resource Measurement) 표준이다. 이 표준 덕분에 기기는 주변 `AP` 목록을 미리 알 수 있어, 기존 방식보다 빠르게 최적의 `AP`로 로밍(roaming)할 수 있다.
