...
> Alfred Workflow는 `Alfred` 앱의 기능 중 하나로, 반복적인 작업을 자동화할 수 있도록 도와준다.
...
이렇게 하면 iPhone에서 받은 2FA 문자 메시지를 Mac에서도 확인할 수 있으며, Workflow가 이를 자동으로 읽을 수 있게 된다.

## 2.2 사용 전 준비사항

`2FA Read Code` Workflow를 사용하려면 Node.js와 SQLite3가 필요하다. `brew`를 이용해 간단히 설치할 수 있다.

### 2.2.1 Homebrew로 Node.js, SQLite3 설치
...
![OTP 코드 입력](image-20250308153829628.png)

이제 더 이상 핸드폰을 직접 확인하고 수동으로 OTP를 입력할 필요가 없다!

# 4. 마무리

2FA Read Code Workflow를 활용하면 OTP 코드 입력을 자동화하여 로그인 과정을 훨씬 편리하게 만들 수 있다. 특히 매일 VPN 접속이나 2FA 인증을 해야 하는 사용자라면, 이 간단한 자동화만으로도 반복 작업을 꽤 줄일 수 있다.

맥북을 쓰면서 OTP 입력이 번거로웠다면 한번 써보는 걸 추천한다.
