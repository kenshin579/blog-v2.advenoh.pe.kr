    ...
    <div className="col-span-12 flex flex-col rounded-card-xl bg-bento-accent p-6 text-white md:col-span-4 md:p-7">
      ...
      <ul className="mt-auto flex flex-col gap-1 pt-6">
        {items.map((it, i) => (
          <li
            key={it.slug}
            className={`py-1.5 ${i > 0 ? 'border-t border-white/20' : ''}`}
          >
            <Link
            ...
            >
              <span className="flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-full bg-white/20 text-[10px] font-bold">
                {i + 1}
              </span>
              <span className="line-clamp-1 text-[13px]">{it.title}</span>
            </Link>
          </li>
        ))}
      </ul>
