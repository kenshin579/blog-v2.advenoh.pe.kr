...
치환은 **렌더링이 끝난 HTML 문자열**에서 수행한다. 같은 함수에 이미 있는 이미지 경로 rewrite(`lib/markdown.ts:63-66`) 바로 다음 줄, 같은 계층이다.

> **설계 변경 (2026-07-26, 코드 리뷰 반영).** 최초 설계는 파싱 **전** markdown 문자열 단계에서 치환하는 것이었다. 이유는 "주석 보존 동작에 의존하지 않는 쪽이 안전하다"였는데, 이는 잘못된 판단이었다. 파싱 전 치환은 정규식이 마크다운 문법을 모르기 때문에 **코드펜스 안의 마커 예시까지 실제 임베드로 치환한다.** 이 기능 자체를 소개하는 글에서 사용법을 코드블록으로 보여주는 순간 재현된다.
>
> 파싱 후 치환은 이 문제가 없다. 블록 레벨 HTML 주석 `<!-- slides -->`는 파이프라인을 통과해 최종 HTML에 리터럴 그대로 살아남는 반면, 코드펜스 안의 같은 문자열은 `&#x3C;!-- slides -->`로 엔티티 인코딩되어 정규식에 걸리지 않는다. 둘이 자동으로 구분된다.

iframe `title` 속성에는 frontmatter의 글 제목을 넣는다. 제목에 `"`나 `&`가 들어갈 수 있으므로 속성에 넣기 전 HTML 이스케이프한다. 파싱 후 치환이므로 rehype의 재직렬화를 거치지 않아 `&quot;`가 그대로 출력에 남는다(파싱 전 치환이었다면 `&#x22;`가 됐을 것이다).
...
사람이 읽는 텍스트(iframe `title` 접미사, 링크 문구, 조작 안내)는 **`lib/i18n/dictionaries.ts`의 `Dict`에 `slides` 섹션으로 넣고 `getDictionary(lang)`으로 가져온다.** `Dict` 타입이 ko/en 양쪽 모양을 강제하므로 한쪽을 빠뜨리면 컴파일 에러가 난다. (최초 설계는 문구를 한국어로 하드코딩했는데, `src`만 `/en/`으로 분기하고 텍스트는 한국어가 나오는 상태였다. 코드 리뷰에서 지적되어 수정했다.)
...
`cleanContent`(29-36행)가 코드블록·이미지·링크는 정리하지만 **HTML 태그는 지우지 않는다.** 인덱스 188건 중 56건의 본문에 꺾쇠가 남아 있고, 그중 `<img ...>` 태그는 36회 출현한다. 코드블록 제거 뒤에 주석 제거와 태그 제거를 추가한다. 코드블록을 먼저 지우므로 코드 안의 꺾쇠는 영향을 받지 않는다.

**태그 제거 정규식은 `<[^>]+>`가 아니라 `<\/?[a-zA-Z][a-zA-Z0-9-]*(?:\s[^>]*)?\/?>`를 쓴다.** 전자는 닫는 `>`가 없는 `<`를 만나면 그 다음에 나오는 아무 `>`까지 통째로 삼킨다. 실제로 `contents/spring/simple-spring-memcached-ssm란/`의 `List<Product)` 오타 하나가 한참 뒤의 `>`와 짝지어져 **정상 텍스트 3672자(그 문서의 29%)가 검색 인덱스에서 소실**됐다. `contents/java/자바8-compable-comparator-차이점/`에서도 `this < o` 때문에 244자가 사라졌다. `<` 다음 첫 글자가 알파벳이나 `/`일 때만 태그로 인정하면 `a < b`, `List<Product)` 같은 본문 표현이 보존된다.
...
## 미확인 지점 / 리스크 — 검증 결과 (2026-07-26 갱신)

- ~~`public/{글폴더}/slides/`와 라우트 산출물 `out/{글폴더}/index.html`이 같은 디렉토리를 공유한다~~ → **해소됨.** 충돌하지 않는다. `out/{글폴더}/`에 Next가 만든 `index.html`·`__next.*` 파일과 우리 `slides/` 디렉토리가 나란히 공존한다. 빌드는 정적 페이지 2287개를 경고·에러 없이 통과했다. 차선책(`/slides/{글폴더}/`)은 쓰지 않았다
- ~~`next dev`가 `public/` 하위 디렉토리의 `index.html`을 자동 해석하는지 불확실하다~~ → **해소됨.** `npm run start`로 서빙했을 때 `/{글폴더}/slides/`가 200으로 열린다. 디렉토리 인덱스 해석이 정상 동작한다
- 글 폴더명이 기존 라우트(`series`, `tags`, `posts`, `en` 등)와 충돌하면 문제가 되지만, 그런 이름의 글은 지금도 `/{글폴더}/` 라우트와 충돌하므로 새로 생기는 위험이 아니다

## 알려진 한계 / 팔로우업 (최종 리뷰에서 발견)

정상 경로에는 지장이 없고, 전부 경계 사례에서 **경고 없이 조용히 열화되는** 종류다.

- **RSS는 이 기능을 모른다.** `scripts/generators/rss.ts`는 `lib/markdown.ts`와 별개의 세 번째 markdown→HTML 파이프라인이고 `allowDangerousHtml`·`rehypeRaw` 없이 동작한다. 그래서 RSS 항목에서 `<!-- slides -->` 마커가 그냥 사라진다(에러도 리터럴 노출도 아님). 구독자는 슬라이드 존재를 알 수 없다. 같은 이유로 기존 `<details>` 블록도 RSS에서 사라지고 있었으므로 이 기능이 만든 회귀는 아니다
- **짝 검사 사각지대.** `generate-content-manifest.ts`의 검사는 `index.md`/`index_en.md`가 **존재할 때만** 돈다. 한국어 전용 글에 `slides_en.html`만 두면 경고 없이 `public/en/{글폴더}/slides/`에 고아 페이지가 배포된다(sitemap에는 안 들어가므로 SEO 노출은 없다)
- **마커는 있고 파일이 없으면** 경고만 뜨고 빌드는 통과한다(설계상 의도). 존재하지 않는 경로를 가리키는 iframe이 렌더된다. 경고가 빌드 로그에 묻히기 쉬우므로 `process.exitCode` 설정을 고려할 만하다
- **마커 오용은 검사하지 않는다.** 문단 중간에 인라인으로 넣으면 `<p>` 안에 `<div>`가 중첩된 무효 HTML이 되는데, 검사 정규식은 "존재 여부"만 보고 "자기 줄 단독 배치"는 보지 않는다
- **데크가 작은 컨테이너를 감당해야 한다 (해결됨).** 임베드는 720×405로 렌더된다. grafana 1편 데크는 1280×720 무대를 `transform-origin: center`로 축소하고 grid 중앙 정렬에 맡기고 있었는데, 무대가 뷰포트보다 크면 브라우저가 safe alignment로 좌상단에 고정해 축소 결과가 오른쪽 아래로 쏠렸다(720×405에서 x 280px, y 184px). `transform-origin: 0 0` + `fit()`에서 직접 translate 하도록 고쳤다. 임베드가 아니라 데크 쪽 버그였고 작은 창 단독 열람에서도 재현됐다. **앞으로 만들 데크도 같은 제약을 지켜야 한다** — `CLAUDE.md`에 기록
- **"앞뒤 빈 줄 필수"는 실제보다 보수적인 문구다.** 실측 결과 마커가 자기 줄에만 있으면 빈 줄 없이 문단이 바로 붙어도 정상 렌더된다(CommonMark HTML 주석 블록은 문단을 인터럽트할 수 있다). 문서가 틀린 건 아니라 그대로 둔다
