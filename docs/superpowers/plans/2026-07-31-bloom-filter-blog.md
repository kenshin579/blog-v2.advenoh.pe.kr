...
표 아래에 한 줄 덧붙인다: 목표 FPR을 10배 낮출 때마다 원소당 약 4.8비트가 더 필요하고, 메모리는 선형으로만 늘어난다.

**3.4**: `map[string]struct{}`와의 메모리 비교. **Task 5 Step 7의 `TestMemory_map과_BloomFilter_비교` 로그 출력을 그대로 옮긴다.**

| 방식 | 100만 개 URL 힙 사용량 |
|------|---------------------|
| `map[string]struct{}` | (로그 값) |
| Bloom Filter (p=0.01) | (로그 값) |

차이가 나는 이유를 한 문단으로 정리한다: `map`은 키 문자열 자체를 계속 들고 있어야 하지만 Bloom Filter는 원소를 저장하지 않고 비트만 남긴다. 그래서 원소가 길어질수록 격차가 벌어지고, 반대로 원소를 다시 꺼내야 하는 상황에서는 Bloom Filter를 쓸 수 없다(6.5절에서 다시 짚는다).
...
- **4.3**: `hashes` 함수. 해시 함수를 k개 만들지 않고 하나를 쪼개 쓰는 Kirsch-Mitzenmacher 기법. **`h2 |= 1`로 홀수를 보장하는 이유를 반드시 설명한다** — h2가 0이면 모든 인덱스가 같은 위치가 되고, 짝수면 도달 가능한 인덱스가 절반으로 줄어 실제 FPR이 이론값보다 나빠진다. h1이 32비트라 m이 2^32를 넘으면 i=0 인덱스가 앞쪽에 몰리는 한계도 한 줄 언급한다.
...
- **5.2**: **Task 5 Step 5의 벤치마크 출력을 그대로 표로 옮긴다.** 수치를 추정하지 않는다.
...
| Add | bits-and-blooms | | | |
| Contains | 직접 구현 | | | |
| Contains | bits-and-blooms | | | |

- **5.3**: 판단 기준. 직접 구현은 원리를 이해하고 싶을 때, 직렬화·동시성·영속화가 필요 없을 때. 라이브러리는 실무 기본값 — 검증된 해시, `WriteTo`/`ReadFrom` 직렬화, 스케일 조정 기능이 붙어 있다.
