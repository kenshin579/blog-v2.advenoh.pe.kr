...
> **실제로 일어난 일 (2026-07-26)**: 위 폴백 **둘 다 적용해도 768px 줄바꿈이 해결되지 않았다.** 라벨 길이 문제가 아니라 768px에 pill 5개 + 검색 + 언어 + 테마 + RSS를 모두 넣는 구조 자체가 원인이었다(항목을 늘리기 전에도 영문에서 RSS가 잘리고 있었다).
>
> 최종 구현은 **헤더 분기점을 `md`→`lg`로 이동**(`components/site-header.tsx`의 `md:` 7곳: pill nav · 검색 pill · 모바일 검색 아이콘 · 언어 전환 · RSS pill · RSS 아이콘 · 햄버거)이고, 폴백 둘은 되돌렸다. 검증은 360 · 768 · 1023 · 1024 · 1280px(ko)과 1024px(en)에서 했다.
>
> **잔여 문제**: 본문 영역은 여전히 `md`에서 전환된다(`components/posts/category-rail.tsx`, `posts-page-client.tsx`, `components/home/*`, 글 페이지 TOC). 768~1023px에서 헤더만 모바일이고 본문은 데스크톱 다단이다. 정렬하려면 그 컴포넌트들도 `lg`로 옮겨야 한다 — 범위 밖이라 후속으로 남긴다.
