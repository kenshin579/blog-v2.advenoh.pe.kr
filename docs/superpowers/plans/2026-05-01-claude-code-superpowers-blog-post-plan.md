# "Claude Code Superpowers 완벽 가이드" 블로그 포스트 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** "Claude Code Superpowers 완벽 가이드: brainstorm부터 PR까지" 블로그 포스트를 작성해 `blog-v2.advenoh.pe.kr/docs/start/claude-code-superpowers-완벽-가이드/index.md`에 게시한다. 약 800줄 분량의 단일 종합 가이드, sister "Claude Code 완벽 가이드" 시리즈에 편입.

**Architecture:** 9개 챕터(개요/Superpowers란/skill 카탈로그/풀 사이클/사례/시작하기/후기/마무리/참고)를 8 phase에 걸쳐 점진 작성. 각 phase 1 commit. 마지막에 PR 생성. 사례(5장)는 PR #701 실제 흐름을 commit log + prompt 발췌 + table 형태로 소개. mermaid 2개 (4장 풀 사이클, 5장 phase 흐름)는 `<br/>` 없이 한 줄 라벨로 작성해 termaid CLI 렌더링과 호환되게 한다.

**Tech Stack:**
- Markdown (CommonMark + YAML frontmatter)
- mermaid for diagrams
- 이미지 1장 권장 (todo 앱 최종 화면 캡처)
- Spec 참조: `blog-v2.advenoh.pe.kr/docs/superpowers/specs/2026-05-01-claude-code-superpowers-blog-post-design.md`

**Scope reference**: 모든 작업은 `/Users/user/src/workspace_blog3/blog-v2.advenoh.pe.kr/`. git 명령은 그 디렉토리에서 실행.

**커밋 규칙 (블로그 저장소도 한국어 컨벤션 따름):**
- 형식: `[feat/superpowers-blog] <type>: <간결한 한국어 설명>`
- 타입: docs (대부분), feat (선택), chore
- 모든 커밋은 `feat/superpowers-blog` 브랜치에 적재

**참고 자료 (모든 phase에서 implementer가 참조 가능)**:
- 실제 진행한 PR: https://github.com/kenshin579/tutorials-go/pull/701 (Todo 풀 구현), https://github.com/kenshin579/tutorials-go/pull/702 (FE 테마)
- spec/plan 파일들은 `tutorials-go/ai/superpowers/todo/docs/superpowers/{specs,plans}/`에 보관
- sister 포스트: `blog-v2.advenoh.pe.kr/contents/ai/claude-code-{mcp-추천-가이드,plugin-hooks-완벽-가이드,확장-기능-완벽-가이드-command-skill-subagent}/index.md`
...
**Files:**
- Existing: `docs/superpowers/specs/2026-05-01-claude-code-superpowers-blog-post-design.md` (작성 완료)
- Existing: `docs/superpowers/plans/2026-05-01-claude-code-superpowers-blog-post-plan.md` (본 문서)

- [ ] **Step 1: blog-v2 디렉토리에서 현재 git 상태 확인**
...
Expected: main 브랜치, untracked `docs/superpowers/specs/...`, `docs/superpowers/plans/...` 두 파일.

- [ ] **Step 2: 피처 브랜치 생성**

```bash
cd /Users/user/src/workspace_blog3/blog-v2.advenoh.pe.kr
git checkout -b feat/superpowers-blog
```

Expected: `Switched to a new branch 'feat/superpowers-blog'`

- [ ] **Step 3: spec + plan 커밋**

```bash
cd /Users/user/src/workspace_blog3/blog-v2.advenoh.pe.kr
git add docs/superpowers/specs/2026-05-01-claude-code-superpowers-blog-post-design.md \
        docs/superpowers/plans/2026-05-01-claude-code-superpowers-blog-post-plan.md
git commit -m "$(cat <<'EOF'
[feat/superpowers-blog] docs: superpowers 블로그 포스트 spec/plan 추가

* 단일 종합 가이드 ~800줄, "Claude Code 완벽 가이드" 시리즈 편입
* PR #701 Todo 사례 + honest 후기 섹션 분리 구조
...
## Phase 0: 디렉토리 + frontmatter + 1장 (개요)

### Task 0: 포스트 디렉토리 생성, frontmatter 작성, 1장 개요

**Files:**
- Create: `docs/start/claude-code-superpowers-완벽-가이드/index.md`

- [ ] **Step 1: 디렉토리 생성**

```bash
mkdir -p /Users/user/src/workspace_blog3/blog-v2.advenoh.pe.kr/docs/start/claude-code-superpowers-완벽-가이드
```

- [ ] **Step 2: `index.md` 작성 — frontmatter + 1장**

`/Users/user/src/workspace_blog3/blog-v2.advenoh.pe.kr/docs/start/claude-code-superpowers-완벽-가이드/index.md` EXACT 시작 내용:
...
이 시점에선 1장만 작성. 2-9장은 후속 phase에서 append.

- [ ] **Step 3: frontmatter + 인코딩 검증**

```bash
file -I /Users/user/src/workspace_blog3/blog-v2.advenoh.pe.kr/docs/start/claude-code-superpowers-완벽-가이드/index.md
head -25 /Users/user/src/workspace_blog3/blog-v2.advenoh.pe.kr/docs/start/claude-code-superpowers-완벽-가이드/index.md
```

Expected: charset=utf-8, frontmatter `---` 두 줄 + tags 13개 + series 1개.

- [ ] **Step 4: 커밋**

```bash
cd /Users/user/src/workspace_blog3/blog-v2.advenoh.pe.kr
git add docs/start/claude-code-superpowers-완벽-가이드/index.md
git commit -m "$(cat <<'EOF'
[feat/superpowers-blog] docs: 포스트 frontmatter + 1장 개요 작성
...
## Phase 1: 2장 (Superpowers란)

### Task 1: Superpowers의 위치 및 sister 포스트 cross-link

**Files:**
- Modify: `docs/start/claude-code-superpowers-완벽-가이드/index.md` (append 2장)

- [ ] **Step 1: 기존 파일 끝에 2장 APPEND**

`index.md` 끝에 다음 내용을 추가:
...
```bash
wc -l /Users/user/src/workspace_blog3/blog-v2.advenoh.pe.kr/docs/start/claude-code-superpowers-완벽-가이드/index.md
```

Expected: ~110-130줄 (frontmatter ~22 + 1장 ~30 + 2장 ~80).
...
git add docs/start/claude-code-superpowers-완벽-가이드/index.md
git commit -m "$(cat <<'EOF'
[feat/superpowers-blog] docs: 2장 Superpowers란 (위치/차이/필요성)
...
## Phase 2: 3장 (핵심 Skill 카탈로그)

### Task 2: 8개 핵심 skill을 표 + 단락으로 설명

**Files:**
- Modify: `docs/start/claude-code-superpowers-완벽-가이드/index.md` (append 3장)

- [ ] **Step 1: 3장을 파일 끝에 APPEND**
...
git add docs/start/claude-code-superpowers-완벽-가이드/index.md
git commit -m "$(cat <<'EOF'
[feat/superpowers-blog] docs: 3장 핵심 Skill 카탈로그 (8개 skill 표 + 단락)
...
## Phase 3: 4장 (풀 사이클 흐름) + mermaid

### Task 3: 풀 사이클 다이어그램 + skill 간 호출 관계 설명

**Files:**
- Modify: `docs/start/claude-code-superpowers-완벽-가이드/index.md` (append 4장)
...
mermaid 다이어그램이 제대로 작성됐는지 termaid로 렌더해 본다.

```bash
sed -n '/^```mermaid$/,/^```$/p' /Users/user/src/workspace_blog3/blog-v2.advenoh.pe.kr/docs/start/claude-code-superpowers-완벽-가이드/index.md | sed '1d;$d' > /tmp/diag-fullcycle-check.mmd
termaid /tmp/diag-fullcycle-check.mmd 2>&1 | head -10
```

Expected: brainstorming → writing-plans → subagent-driven-development → requesting-code-review → finishing-a-development-branch 5개 노드가 가로로 연결된 박스 다이어그램.
...
git add docs/start/claude-code-superpowers-완벽-가이드/index.md
git commit -m "$(cat <<'EOF'
[feat/superpowers-blog] docs: 4장 풀 사이클 흐름 + mermaid 다이어그램

* 5개 핵심 skill 흐름도, 사용자 개입 6 지점 표
* subagent-driven vs executing-plans 추적 방식 차이
* 디렉토리 구조 안내
...
Expected: ~310-340줄 (이전 ~250 + 4장 ~80).

---

## Phase 4: 5장 (사례 — Todo 풀 사이클, 메인 비중)

### Task 4: PR #701 사례를 5단계로 따라가기

이 phase가 가장 큰 비중. 5.1~5.5를 한 번에 작성.

**Files:**
- Modify: `docs/start/claude-code-superpowers-완벽-가이드/index.md` (append 5장)
...
NOTE: 5장에 mermaid 블록이 2개 들어 있어 outer markdown fence는 4-backtick(```` ```` ````)으로 작성했다. 같은 패턴이 Phase 3/5에도 적용되어 있다.

- [ ] **Step 2: 5장 mermaid 다이어그램 검증**

```bash
# 5장에 mermaid 2개가 있는지 확인
grep -c "^```mermaid$" /Users/user/src/workspace_blog3/blog-v2.advenoh.pe.kr/docs/start/claude-code-superpowers-완벽-가이드/index.md
```

Expected: 4 (1장 0 + 4장 1 + 5장 2 + 다른 곳 1) — 정확한 수치는 phase 진행 따라 다름. 최소 3 이상이어야 함.
...
git add docs/start/claude-code-superpowers-완벽-가이드/index.md
git commit -m "$(cat <<'EOF'
[feat/superpowers-blog] docs: 5장 사례 — Todo 풀 사이클 (5.1~5.5)

* brainstorm 7문항 + spec 자동생성
* plan 11 phase 분해
* subagent-driven 25 commits + 두 단계 리뷰가 잡은 critical 이슈 2건
* MCP playwright 라이브 e2e + spec 영속화
* PR 생성/머지 명령 흐름
...
**Files:**
- Modify: `docs/start/claude-code-superpowers-완벽-가이드/index.md` (append 6장)
...
git add docs/start/claude-code-superpowers-완벽-가이드/index.md
git commit -m "$(cat <<'EOF'
[feat/superpowers-blog] docs: 6장 시작하기 (설치/첫 사이클/사전 준비/디렉토리)
...
## Phase 6: 7장 (Honest 후기)

### Task 6: 좋았던 점 / 비용 / 함정 / 권장 시나리오

**Files:**
- Modify: `docs/start/claude-code-superpowers-완벽-가이드/index.md` (append 7장)
...
git add docs/start/claude-code-superpowers-완벽-가이드/index.md
git commit -m "$(cat <<'EOF'
[feat/superpowers-blog] docs: 7장 honest 후기 (좋았던점/비용/함정/권장)

* phase 분리 + 두 단계 리뷰 + 회귀 발견 등 좋았던 점
* per-task 3회 디스패치 비용, plan 체크박스 미갱신 같은 트레이드오프
* rune count, segmented control e2e 등 실제 함정
* 적합 시나리오 표 + 미사용 skill 추천
...
## Phase 7: 8장 (마무리) + 9장 (참고)

### Task 7: takeaway + 외부/내부 링크

**Files:**
- Modify: `docs/start/claude-code-superpowers-완벽-가이드/index.md` (append 8, 9장)
...
- [Superpowers GitHub 저장소](https://github.com/anthropics/superpowers) (정확한 URL은 marketplace 페이지에서 확인)
...
git add docs/start/claude-code-superpowers-완벽-가이드/index.md
git commit -m "$(cat <<'EOF'
[feat/superpowers-blog] docs: 8장 마무리 + 9장 참고 (포스트 본문 완성)

* takeaway 3줄 + 다음 시도 추천
* 외부 링크 + sister 포스트 cross-link
...
## Phase 8: 검증 + 이미지 (선택) + PR 생성

### Task 8: 종합 검증, 이미지 추가, PR 생성

**Files:**
- Modify (선택): `docs/start/claude-code-superpowers-완벽-가이드/index.md` (이미지 삽입 시)
- Add (선택): `docs/start/claude-code-superpowers-완벽-가이드/todo-app-final.png`

- [ ] **Step 1: 인코딩 + 줄 수 + frontmatter 종합 검증**
...
head -25 /Users/user/src/workspace_blog3/blog-v2.advenoh.pe.kr/docs/start/claude-code-superpowers-완벽-가이드/index.md
echo "--- heading count ---"
grep -c "^# [0-9]\." /Users/user/src/workspace_blog3/blog-v2.advenoh.pe.kr/docs/start/claude-code-superpowers-완벽-가이드/index.md
echo "--- mermaid count ---"
grep -c "^```mermaid$" /Users/user/src/workspace_blog3/blog-v2.advenoh.pe.kr/docs/start/claude-code-superpowers-완벽-가이드/index.md
```
...
- charset=utf-8
- 800줄 부근
- frontmatter 첫 줄 `---`, series 키 포함
- 9개 top-level heading (`# 1. 개요` ~ `# 9. 참고`)
- mermaid 다이어그램 최소 2개

- [ ] **Step 2: mermaid 다이어그램 termaid 검증**

```bash
# 4장 다이어그램만 추출해 termaid에 통과시킴
sed -n '/^# 4\./,/^# 5\./p' /Users/user/src/workspace_blog3/blog-v2.advenoh.pe.kr/docs/start/claude-code-superpowers-완벽-가이드/index.md \
  | sed -n '/^```mermaid$/,/^```$/p' | sed '1d;$d' > /tmp/diag-ch4.mmd
termaid /tmp/diag-ch4.mmd 2>&1 | head -10
echo "--- 5장 첫번째 다이어그램 ---"
sed -n '/^# 5\./,/^# 6\./p' /Users/user/src/workspace_blog3/blog-v2.advenoh.pe.kr/docs/start/claude-code-superpowers-완벽-가이드/index.md \
  | sed -n '/^```mermaid$/,/^```$/p' | sed '1d;$d' | head -7 > /tmp/diag-ch5.mmd
termaid /tmp/diag-ch5.mmd 2>&1 | head -10
```

Expected: 두 다이어그램 모두 Unicode art 박스로 정상 렌더 (오류 없이).

- [ ] **Step 3: 빌드 (옵션 — Vite 환경에서 빌드 가능한 경우)**

```bash
cd /Users/user/src/workspace_blog3/blog-v2.advenoh.pe.kr
npm run build 2>&1 | tail -10 || echo "build skipped (run on demand)"
```

Expected: 에러 없이 빌드. 빌드 환경 문제 시 skip 후 dev에서 시각 확인.

- [ ] **Step 4: 이미지 추가 (선택)**

`/tmp/`에 남아있는 todo 앱 캡처(예: `fe-theme-final-2026-04-30T14-04-02-928Z.png`) 또는 새로 캡처한 이미지를 포스트 디렉토리에 복사.

```bash
LATEST_IMG=$(ls -t /tmp/fe-theme-final-*.png 2>/dev/null | head -1)
if [ -n "$LATEST_IMG" ]; then
  cp "$LATEST_IMG" /Users/user/src/workspace_blog3/blog-v2.advenoh.pe.kr/docs/start/claude-code-superpowers-완벽-가이드/todo-app-final.png
  echo "이미지 복사 완료"
else
  echo "이미지 없음 — Step 4 skip"
fi
```

이미지가 복사됐다면 5.4 끝에 다음 한 줄을 INSERT (5.4의 마지막 단락 아래에):

```markdown

![Todo 앱 최종 화면 — 헤더 카운트 / segmented 필터 / priority 색 뱃지 / 완료 line-through / 마감일 subtext](todo-app-final.png)
```

이미지 없이 진행해도 됨. 본 step은 선택.

- [ ] **Step 5: 커밋 (이미지 추가 시에만)**

```bash
cd /Users/user/src/workspace_blog3/blog-v2.advenoh.pe.kr
if [ -f docs/start/claude-code-superpowers-완벽-가이드/todo-app-final.png ]; then
  git add docs/start/claude-code-superpowers-완벽-가이드/todo-app-final.png \
          docs/start/claude-code-superpowers-완벽-가이드/index.md
  git commit -m "$(cat <<'EOF'
[feat/superpowers-blog] docs: 5.4장에 todo 앱 최종 화면 캡처 추가
...
이미지 없이 진행 시 본 step skip.

- [ ] **Step 6: PR 생성 (사용자 컨펌 후)**

```bash
cd /Users/user/src/workspace_blog3/blog-v2.advenoh.pe.kr
git push -u origin feat/superpowers-blog
gh pr create --title "feat: Claude Code Superpowers 완벽 가이드 (블로그 포스트)" --body "$(cat <<'EOF'
## Summary

`docs/start/claude-code-superpowers-완벽-가이드/index.md` 신규 포스트 추가. ~800줄, "Claude Code 완벽 가이드" 시리즈에 편입.

## 주요 내용

- 1-2장: superpowers 위치 (Plugin marketplace) + 다른 Skill 가이드와의 차이
- 3장: 핵심 8개 skill 카탈로그 (brainstorming/writing-plans/subagent-driven 등)
- 4장: 풀 사이클 흐름 mermaid + 사용자 개입 6 지점
- 5장: 사례 — `tutorials-go` Todo 풀 사이클 (PR #701) 단계별 follow-along
- 6장: 시작하기 (설치, 첫 사이클, CLAUDE.md 정책 권장)
- 7장: honest 후기 (좋았던 점 / 비용 / 함정 / 권장 시나리오)
- 8-9장: takeaway + 외부/sister 포스트 링크

## 출처

- 실제 진행 PR: [tutorials-go #701](https://github.com/kenshin579/tutorials-go/pull/701), [#702](https://github.com/kenshin579/tutorials-go/pull/702)
- spec 문서: `docs/superpowers/specs/2026-05-01-claude-code-superpowers-blog-post-design.md`
- plan 문서: `docs/superpowers/plans/2026-05-01-claude-code-superpowers-blog-post-plan.md`

## Test plan

- [ ] 마크다운 렌더링 확인 (mermaid 2개, 표 ~7개, 코드 블록 다수)
- [ ] frontmatter `series: "Claude Code 완벽 가이드"` 시리즈 페이지에 노출
- [ ] sister 포스트 cross-link 동작
- [ ] 외부 링크 (PR #701, #702) 동작
...
- [ ] Pre-flight 완료 (피처 브랜치 + spec/plan 커밋)
- [ ] Phase 0 완료 (frontmatter + 1장)
- [ ] Phase 1 완료 (2장)
- [ ] Phase 2 완료 (3장)
- [ ] Phase 3 완료 (4장 + mermaid 1개)
- [ ] Phase 4 완료 (5장 사례 5.1~5.5 + mermaid 2개)
- [ ] Phase 5 완료 (6장)
- [ ] Phase 6 완료 (7장)
- [ ] Phase 7 완료 (8, 9장)
- [ ] Phase 8 완료 (검증 + 이미지(옵션) + PR 생성)

각 phase는 별도 git commit으로 분리. 최종 분량 ~800줄, mermaid 2개, 표 7-8개, sister 포스트 cross-link 3개.
