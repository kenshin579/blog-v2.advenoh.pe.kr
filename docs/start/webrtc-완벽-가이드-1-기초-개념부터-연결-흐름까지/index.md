...
**SFU**는 **Selective Forwarding Unit**의 약자로, 다자간 통화에서 각 참가자의 미디어 스트림을 서버가 받아서 다른 참가자들에게 **선택적으로 전달(forwarding)**하는 중계 서버 아키텍처다. 미디어를 디코딩/인코딩하지 않고 그대로 포워딩하므로 서버 부하가 낮고, 대규모 화상 회의에 적합하다. Zoom, Google Meet 등 대부분의 화상 회의 서비스가 SFU 방식을 사용한다. 자세한 내용은 편 3에서 다룬다.
