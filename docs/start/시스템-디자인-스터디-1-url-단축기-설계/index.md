...
그래서 캐시를 **cache-aside** 방식으로 둔다. 웹 서버가 캐시를 먼저 보고, 있으면 그대로 응답한다. 없으면 DB에서 읽어 캐시에 채운 뒤 응답한다. 어떤 키를 미리 올려둘지 고를 필요 없이 실제로 조회된 키가 스스로 올라오는 구조라, 인기 링크는 자연히 남고 잊힌 링크는 LRU로 밀려난다. 게다가 매핑은 한 번 정해지면 바뀌지 않는다. 2.1에서 수정과 만료를 범위 밖으로 둔 덕에 캐시에 올라간 값이 낡을 일이 없고, 캐시 무효화라는 골치 아픈 문제가 이 서비스에서는 거의 발생하지 않는다.
...
이 절의 결과는 2.2의 낮은 지연으로 돌아간다. 캐시가 적중하면 리다이렉트 응답은 메모리 조회 한 번과 헤더 생성으로 끝난다. 리다이렉트는 사용자가 링크를 누른 뒤 목적지 페이지가 뜨기까지의 경로 한가운데 있으므로, 여기서 줄인 시간은 그대로 체감 로딩 시간에서 빠진다. 캐시 적중률이 곧 이 요구사항의 달성도가 된다.
