...
여러 Kubernetes 클러스터를 오가며 일하다 보면, 하루에도 수십 번 이 명령어를 친다.
...
# 다른 클러스터로 전환
kubectl config use-context arn:aws:eks:ap-northeast-2:123456789:cluster/prod-cluster

# 작업할 namespace 지정 (매 명령어마다...)
...
1. **context 이름이 너무 길다.** EKS나 NCP 같은 매니지드 Kubernetes의 context 이름은 ARN이나 긴 식별자라 외워서 칠 수가 없다. 결국 `kubectl config get-contexts`로 목록을 보고 → 복사 → 붙여넣기를 반복한다.
2. **namespace를 매번 `-n`으로 붙여야 한다.** `kubectl config set-context --current --namespace=...`로 기본 namespace를 바꿀 수는 있지만, 이것도 namespace 이름을 정확히 알아야 하고 명령어가 길다.
...
fzf의 동작은 간단하다. 어떤 명령의 출력을 파이프로 넘기면 선택 UI가 뜨고, 고른 줄이 그대로 출력된다.
...
- `fzf ...` — 그 이름 목록을 fzf로 넘겨 대화형으로 고른다.
...
| `--no-mouse` | 마우스 비활성화 (터미널 스크롤이 fzf에 먹히지 않게) |

마지막으로 `[[ $context != "" ]]` 가드는 **fzf에서 ESC로 취소했을 때**(빈 값) `use-context`가 실행되지 않게 막아준다.
...
> minikube                                                     ← 현재 커서(>) 위치
  ...
  arn:aws:eks:ap-northeast-2:123456789:cluster/staging-cluster
```

여기서 `prod` 몇 글자만 타이핑하면 목록이 즉시 좁혀진다. 긴 ARN을 외우거나 복사할 필요가 없다.
...
```bash
$ kx
Switched to context "arn:aws:eks:ap-northeast-2:123456789:cluster/prod-cluster".
```

## 4. ns — fzf로 namespace 전환하기
...
`ns`를 실행하면 현재 클러스터의 namespace 목록이 fzf로 뜬다.
...
`ns`가 선택한 namespace를 `~/.ns`에 저장해두는 이유는, 그 값을 **단일 소스(single source of truth)**로 삼아 alias와 엮기 위해서다. `~/.zshrc`(또는 `~/.bashrc`)에 다음 두 줄을 둔다.
...
핵심은 **kubectl이 "현재 상태"를 어디에 기억하느냐**의 차이다.
...
| --- | --- | --- | --- |
| **context** | kubeconfig `current-context` (kubectl이 자동 사용) | 없음 | ❌ |
...
`~/bin`을 PATH에 등록해두면 어디서나 `kx`, `ns`만 쳐도 실행된다.
...
정리하면, 길고 외우기 힘든 명령어가 이렇게 짧아졌다.

| 전 | 후 |
| --- | --- |
| `kubectl config use-context arn:aws:eks:...:cluster/prod` | `kx` |
