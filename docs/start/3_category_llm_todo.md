...
- [x] 의존성 추가
  ...
  - [x] 의존성 설치: `cd scripts/news && pip install -e .`
...
- [x] `ai_categorizer.py` 파일 생성
  - [x] `scripts/news/category/ai_categorizer.py` 생성
  - [x] 상수 정의 (MODEL, BATCH_SIZE, TEMPERATURE)
  - [x] SYSTEM_PROMPT 작성

- [x] Pydantic 모델 정의 (구조화된 출력)
  - [x] `CategoryType` - Literal 타입으로 카테고리 제한
  - [x] `Classification` - 단일 분류 결과 모델
  - [x] `ClassificationResponse` - 전체 응답 모델

- [x] 핵심 함수 구현
  - [x] `categorize_with_ai()` - 메인 분류 함수
  - [x] `_classify_batch()` - `client.beta.chat.completions.parse()` 사용
  - [x] `_apply_classifications()` - 파싱된 응답 적용

- [x] Fallback 로직 구현
  - [x] `categorize_with_fallback()` - 에러 시 키워드 매칭 전환
  - [x] 예외 처리 (API 실패, refusal 처리 등)
...
- [x] main.py 수정
  - [x] `categorize_with_fallback` import 추가
  - [x] 기존 `categorize_articles` 호출을 `categorize_with_fallback`으로 변경
  - [x] 분류 결과 로깅 추가
...
- [x] 통합 테스트
  ...
  - [x] 예상 결과 확인:
    ...
    - AI / ML: 클로드 코드
    - Development: strcpy (Security 대신 Development로 분류됨)
