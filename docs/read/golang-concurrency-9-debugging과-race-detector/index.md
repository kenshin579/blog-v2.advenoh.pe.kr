...
### race condition 예시: 여러 goroutine에서 counter++ 동시 실행

가장 흔한 race condition 예시는 여러 goroutine에서 공유 변수를 동시에 증가시키는 것이다.

```go
// 이 코드는 race condition이 있다!
counter := 0
...
### map concurrent access: sync.Map으로 수정
...
### slice concurrent access: 인덱스별 독립 접근
...
### circular wait deadlock: 두 goroutine이 서로의 lock 대기

가장 전형적인 deadlock 패턴은 두 goroutine이 서로 다른 순서로 lock을 획득하는 것이다.

```go
// 이 코드는 deadlock이 발생할 수 있다!
...
### channel deadlock: 같은 goroutine에서 unbuffered send
...
ch <- 42                 // 받아줄 goroutine이 없으므로 영원히 대기
val := <-ch              // 여기에 도달하지 못함
```

### 수정: buffered channel 또는 별도 goroutine
...
### runtime.NumGoroutine()으로 goroutine 수 모니터링
...
### goroutine leak 탐지 패턴 (baseline 비교)
...
### go test -race: race condition 탐지
...
### GODEBUG=asyncpreemptoff=1: 동기 선점 비활성화
