...
동시성에 대한 주석을 먼저 짚어 둔다. 이 구현은 비트 배열을 락 없이 읽고 쓰므로 여러 goroutine에서 동시에 `Add`를 호출하면 안전하지 않다. 5장에서 볼 `bits-and-blooms/bloom`도 마찬가지라 라이브러리로 바꾼다고 해결되지 않는다. 여러 goroutine이 같은 필터를 쓴다면 호출하는 쪽에서 락을 걸거나, 6.2절처럼 필터를 프로세스 밖(Redis 등)에 두어야 한다.
...
필드 `n`은 이름과 달리 원소 개수가 아니라 **`Add`를 호출한 횟수**다. 같은 URL을 두 번 넣으면 2로 센다. 필터는 원소를 저장하지 않으니 중복인지 알 방법이 없고, 알려면 필터 자신에게 물어봐야 하는데 그 답에는 false positive가 섞인다. 그래서 세는 쪽을 포기하고 호출 횟수를 그대로 들고 있다. 이 값은 4.5절에서 이론 FPR을 계산할 때 n 자리에 들어간다. 중복 삽입이 많으면 실제보다 비관적인 추정치가 나온다.
...
**실제로 이 한 줄이 막아주는 것은 `h2 == 0` 하나뿐이다.** h2가 0이면 `h1 + i*h2`가 i와 무관하게 h1이 되어, k개 인덱스가 전부 한 자리로 겹친다. 그 원소에 한해 필터는 k=1로 동작한다.

얼마나 나빠지는지는 실측으로 확인할 수 있다. m = 9,585,059 / k = 7 / n = 100만 조건에서 보정을 빼고 h2가 0인 상황을 강제하면 FPR이 **1.0%에서 9.9%로** 무너진다. 이 값은 k=1일 때의 이론값 $1 - e^{-n/m} = 0.0991$과 정확히 맞는다.

h2가 0이 될 확률은 $2^{-32}$라 극히 낮지만, 막는 비용이 OR 연산 한 번이니 그대로 둘 이유가 없다.
...
이 함수에는 하나 더 밝혀 둘 제약이 있다. h1과 h2를 32비트로 잘라 쓰므로 둘 다 $2^{32}$보다 작고, mod를 취하기 전의 `pos = h1 + i*h2`는 $k(2^{32} - 1)$을 넘지 못한다. 따라서 m이 $k \cdot 2^{32}$보다 커지면 비트 배열 뒷부분에는 어떤 입력으로도 도달할 수 없다.

$m > 2^{32}$는 512 MiB 이상이라 이 예제가 다루는 범위 밖이지만, 코드를 가져다 쓸 사람에게는 알려야 할 한계다. 64비트를 알뜰하게 쪼개 쓴 대가가 여기에 있고, 이 대가가 성능으로 어떻게 돌아오는지는 5.2절에서 다시 본다.
...
두 값의 차이는 0.00003, 건수로는 30건이다. 이 차이가 작다고 말하려면 기준이 필요하다.

조회 100만 회는 성공 확률 0.01인 베르누이 시행 100만 번이므로 false positive 건수는 이항분포를 따르고, 표준편차는 $\sqrt{1{,}000{,}000 \times 0.01 \times 0.99} \approx 99.5$건이다. 30건 차이는 $0.3\sigma$에 해당한다.

같은 실험을 반복하면 이 정도 폭은 매번 다르게 나오는 것이 정상이고, 이론값이 이 범위 안에 들어왔다는 것은 3.1절의 근사가 실제 동작과 어긋나지 않는다는 뜻이다.
...
**m은 양쪽이 항상 같지만 k는 다를 수 있다.** m 공식은 똑같고 올림 처리까지 같아서 어떤 p를 넣어도 값이 일치한다.

갈리는 것은 k를 정수로 만드는 방법이다. 라이브러리는 $k = \lceil (m/n) \ln 2 \rceil$으로 올리고, 4장 구현은 $k = \mathrm{round}((m/n) \ln 2)$로 반올림한다. 그래서 p = 0.1에서는 3과 4로, p = 0.0001에서는 13과 14로 갈린다.

p = 0.01에서 둘 다 7이 나온 것은 계산값 6.64가 반올림해도 올림해도 7이 되기 때문이지 두 구현이 같은 규칙을 쓰기 때문이 아니다.
...
흥미로운 것은 세 번째 값이다. 각자 1%씩 틀리는데 같은 키에서 틀리는 경우는 7건뿐이다. 해시 함수가 다르니 어떤 키가 하필 k개 비트를 다 밟는지도 달라진다. 두 오답 집합이 독립이라면 겹칠 기대값은 $100{,}000 \times 0.01 \times 0.01 = 10$건이고, 실측 7건은 그 근처다. 정확도가 같다는 말이 "같은 답을 낸다"는 뜻은 아니라는 것을 보여주는 숫자다.
...
**false positive가 나면 디스크 블록을 한 번 헛읽고 끝난다.** 읽어 본 블록에 키가 없으니 다음 SSTable로 넘어갈 뿐이고, 최종적으로 반환되는 값은 달라지지 않는다. 판정을 내리는 것은 필터가 아니라 실제로 읽은 블록이다.
...
첫 번째는 자료구조의 false negative가 아니라 운영 실수다. 필터에 넣지 않은 원소를 없다고 답하는 것은 정상 동작이고, 쓰기와 필터 갱신이 같은 트랜잭션에 묶이지 않는다는 점도 감안해야 한다.
...
**false positive가 나면 서버 왕복 한 번이 추가된다.** 서버가 전체 해시를 대조해 정답을 주므로 판정 자체는 틀리지 않는다.
...
한 가지 정정할 것이 있다. Chrome이 Safe Browsing에 Bloom Filter를 썼다는 설명은 널리 인용되지만 **과거형이다.** Chromium은 2012년 9월에 Safe Browsing의 로컬 판정을 Bloom Filter에서 prefix set으로 교체했다.

지금의 Safe Browsing Update API는 URL의 SHA-256 해시 프리픽스(보통 4바이트)를 내려받아 로컬에 두고, 프리픽스가 걸리면 서버에 그 프리픽스로 전체 해시 목록을 요청해 대조한다. 교체 이유는 Chromium 코드 리뷰에 적혀 있지 않아 여기서 추측하지 않는다.
...
앞의 세 사례에는 뒤에서 최종 판정을 내려 주는 것이 있었지만 크롤러의 방문 여부 판정에는 그 뒷단이 없다. 필터가 "이미 봤다"고 하면 그게 최종 답이다. 확인하려면 방문한 URL 집합을 정확히 들고 있어야 하는데, 그걸 안 하려고 필터를 쓴 것이다.
...
완화책은 있다. 필터가 차면 하위 필터를 덧붙이는 확장형 변형이고 RedisBloom도 `BF.RESERVE`의 `EXPANSION` 옵션으로 제공한다. 다만 조회할 때 하위 필터를 차례로 봐야 하므로 늘어난 만큼 비용이 붙는다. 예상 원소 수를 제대로 잡는 것의 대체재가 아니라 잘못 잡았을 때의 안전장치다.
