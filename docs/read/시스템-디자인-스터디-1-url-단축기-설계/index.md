...
이 표에서 읽어야 할 것은 각 칸의 값보다 칸들 사이의 관계다. 충돌 처리와 그 아래 딸린 재시도, Bloom filter는 URL 단축기가 원래 필요로 하는 부품이 아니다. 해시 함수를 골라 결과를 7자로 잘랐기 때문에 생긴 문제를 메우는 장치다. 유일 ID 방식을 고르면 그 두 줄이 통째로 비고, 대신 "유일 ID 생성기가 필요하다"는 줄이 채워지면서 4.4에서 본 어려움을 떠안는다. 앞 단계의 선택이 뒤 단계의 필요를 만든다. 3.2에서 302를 고른 탓에 캐시를 전제로 깔게 된 것과 같은 구조다. 그래서 이 비교에서 정해야 할 것은 어느 문제를 떠안을지다.

## 4.6 읽기 편중과 캐시

2.3에서 잡은 읽기 11,600 QPS는 쓰기 1,160 QPS의 열 배다. 그리고 3.2에서 302를 골랐기 때문에 이 열 배가 고스란히 서버에 도착한다. 301이었다면 같은 브라우저의 두 번째 클릭부터는 서버까지 오지 않았을 요청이다. 매핑의 통제권을 쥐는 대가로 읽기 부하를 전부 떠안기로 한 셈이고, 그 부하를 받아낼 자리를 이제 정해야 한다.

다행히 이 부하에는 파고들 구석이 있다. 리다이렉트 요청은 3,650억 건의 링크에 고르게 흩어지지 않는다. 뉴스레터나 SNS 게시물에 실려 널리 퍼진 소수의 링크가 트래픽의 대부분을 만들고, 나머지 대다수는 발급된 뒤 몇 번 눌리고 조용해진다. 10년 치 36.5TB를 통째로 메모리에 올리는 건 불가능하지만 그럴 필요도 없다. 자주 조회되는 상위 일부만 메모리에 있어도 요청의 상당수가 DB까지 내려가지 않고 끝난다.

그래서 캐시를 **cache-aside** 방식으로 둔다. 웹 서버가 캐시를 먼저 보고, 있으면 그대로 응답한다. 없으면 DB에서 읽어 캐시에 채운 뒤 응답한다. 어떤 키를 미리 올려둘지 고를 필요 없이 실제로 조회된 키가 스스로 올라오는 구조라, 인기 링크는 자연히 남고 잊힌 링크는 LRU로 밀려난다. 게다가 매핑은 한 번 정해지면 바뀌지 않는다. 2.1에서 기능을 단축과 리다이렉트 둘로만 좁힌 덕에 캐시에 올라간 값이 낡을 일이 없고, 캐시 무효화라는 골치 아픈 문제가 이 서비스에서는 거의 발생하지 않는다.
...
마지막 노드가 "301 또는 302"로 되어 있는 것은 이 흐름 자체가 상태 코드와 무관하기 때문이다. 캐시를 보고 없으면 DB로 내려가는 경로는 어느 쪽을 쓰든 똑같다. 이 글은 3.2에서 302를 골랐으니 실제로 나가는 응답은 302다.

흐름에서 눈여겨볼 곳은 404 갈래다. 존재하지 않는 짧은 URL로 들어온 요청은 캐시에도 없고 DB에도 없으므로 매번 캐시를 헛돌고 DB까지 내려간다. 오타 한 번이면 그만이지만, 7자 문자열을 무작위로 대입하는 스캔이 들어오면 그 시도 하나하나가 DB 조회로 번역된다. 캐시가 걸러주지 못하는 트래픽이다. 없다는 사실 자체를 짧은 TTL로 캐시에 남겨두거나, 4.2에서 쓰기 쪽 장치로 본 Bloom filter를 읽기 경로에도 두면 존재하지 않는 키를 DB에 닿기 전에 쳐낼 수 있다.

이 절의 결과는 2.2의 낮은 지연으로 돌아간다. 캐시가 적중하면 리다이렉트 응답은 메모리 조회 한 번과 헤더 생성으로 끝난다. 2.2에서 적어둔 대로 이 경로에서 줄인 시간은 그대로 체감 로딩 시간에서 빠진다. 캐시 적중률이 곧 이 요구사항의 달성도가 된다.

## 4.7 전체 흐름 정리

4.1부터 4.6까지 조각으로 나눠 본 것들을 한 장에 이어 붙여본다. 아래 단축 흐름은 **해시 방식(4.1~4.2)을 골랐을 때**의 모습이다. 4.5에서 두 방식 중 어느 쪽도 고르지 않고 열어뒀으므로, 이 그림은 두 갈래 중 한쪽을 펼쳐놓은 것으로 보면 된다.
...
긴 URL이 들어오면 해시를 만들고 앞 7자를 자른다. 그다음이 4.2에서 다룬 유일성 확인이다. Bloom filter가 "없다"고 답하면 그 답은 확실하므로 DB 조회를 건너뛰고 바로 저장한다. "있을 수 있다"고 답할 때만 DB에 실제로 있는지 물어보고, 정말 있으면 사전 정의 문자열을 붙여 다시 해싱하는 루프로 돌아간다. 유일 ID와 Base62를 골랐다면 Bloom filter와 재시도 분기가 통째로 빠지고, 그 앞에 ID를 발급받는 단계가 붙으면서 4.4에서 본 생성기 문제를 떠안는다.

리다이렉트 흐름은 4.6에서 본 그대로다. 캐시를 보고, 없으면 DB에서 찾아 캐시에 채운 뒤 302로 돌려보낸다. 두 흐름을 나란히 놓으면 이 설계가 어디에 값을 치르는지 드러난다. 쓰기 경로의 비용은 유일성을 확인하는 데 몰려 있고, 읽기 경로의 비용은 캐시 적중률 하나에 걸려 있다.

# 5. 4단계: 마무리 - 더 고민할 것들

아래 항목들은 이 글에서 다루지 않았지만, 실제로 운영에 올린다면 하나씩 결정해야 하는 것들이다.

- **DB 샤딩과 복제** — 36.5TB는 한 대로 감당하기 어렵다. 짧은 URL을 기준으로 샤드를 나누고 읽기는 복제본으로 분산한다.
- **만료와 정리** — 영구 보관이 필요 없다면 TTL을 두고 만료된 레코드를 주기적으로 지운다.
- **커스텀 별칭** — 사용자가 원하는 문자열을 지정하는 기능. 별칭 선점 문제와 예약어 처리가 따라온다.
- **rate limiting과 악성 URL 차단** — 단축 API 남용을 막고, 등록되는 링크가 피싱인지 걸러내는 검사가 필요하다.
- **클릭 분석** — 3.2에서 302를 골랐으니 리다이렉트마다 이벤트를 남겨 별도 파이프라인으로 집계할 수 있다.
- **멀티 리전과 CDN** — 지연을 줄이려면 사용자와 가까운 곳에서 리다이렉트를 처리해야 한다.

# 6. 마무리

4단계를 따라오면서 갈림길이 여러 번 나왔는데, 그중 분량을 가장 많이 쓴 것은 세 가지다.

- **301이냐 302냐**(3.2) — 서버 부하를 줄일 것인가, 클릭 분석 여지를 남길 것인가. 302를 골랐고 그 대가로 읽기 부하를 전부 떠안았다.
- **해시냐 유일 ID + Base62냐**(4.1~4.5) — 충돌을 감수하고 처리 장치를 붙일 것인가, 충돌이 생기지 않는 구조를 택할 것인가.
- **분산 환경에서 유일 ID를 어떻게 만들 것인가**(4.4) — 조율 없이 각자 발급할 것인가, 중앙 창구에서 받아올 것인가.

세 갈림길 모두 한쪽을 고르면 다른 쪽이 함께 사라진다. 302를 고른 순간 브라우저 캐시가 걸러주던 요청이 전부 서버로 들어왔고, 해시를 고르면 유일성 확인 조회가 쓰기 경로에 붙는다. 그래서 설계를 정리할 때 더 오래 남겨둘 것은 무엇을 골랐는지보다 그 선택으로 무엇을 포기했는지다. 포기한 쪽을 적어두면 전제가 바뀌었을 때 어느 결정을 다시 열어야 할지 바로 찾을 수 있다.

1장에서 이 4단계를 다음 주제에도 그대로 쓰겠다고 적었다. 다음 편도 요구사항을 좁히고 숫자를 뽑는 데서 시작한다.
